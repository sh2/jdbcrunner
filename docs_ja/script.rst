...
Mozilla RhinoのライブラリーはJdbcRunnerのJARファイルに組み込まれているので、以下のコマンドでJavaScriptの動作確認ができます。
...
ウェブブラウザーで用いられるdocumentオブジェクトなどはありません。
...
#. JdbcRunnerに設定するパラメーターがある場合、パラメーターをグローバル変数として宣言する
...
パラメーターをグローバル変数として宣言する
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

JdbcRunnerでは設定パラメーターをコマンドラインオプションで指定するほかに、特定のグローバル変数を宣言することでも行うことができます。
...
固定的なパラメーターをスクリプト内で設定しておくと便利です。

また、設定パラメーターと関係ないグローバル変数を宣言しても特に問題はありません。
ただし、変数名が設定パラメーター名と重複しないように注意してください。
